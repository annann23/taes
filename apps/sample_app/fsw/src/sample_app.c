...
sample_hk_tlm_t    SAMPLE_HkTelemetryPkt; //hk
CFE_SB_PipeId_t    SAMPLE_CommandPipe;    //
CFE_SB_MsgPtr_t    SAMPLEMsgPtr;
CFE_TBL_Handle_t   MyTableHandle;          //led
LED_MyTable_t	   MyTblInitData;
		...
		for (int i=0;i<10;i++)
{
		...
		sleep(1);
		SAMPLE_HkTelemetryPkt.sample_led1_state+=GPIO_Lib_Read(LED1);
		SAMPLE_HkTelemetryPkt.sample_led2_state+=GPIO_Lib_Read(LED2);		
	...
	int32 Status;
	LED_MyTable_t *MyTblPtr;
	int16 total;

	//Register my table with Table Sevices
	
	Status=CFE_TBL_Register(&MyTableHandle,"MyTableName",sizeof(LED_MyTable_t),CFE_TBL_OPT_DEFAULT,&LED_MyTableValidationFunc);

	//Load my table with Data from Memory
	Status=CFE_TBL_Load(MyTableHandle,SrcType,&SAMPLE_HkTelemetryPkt);

	Status=CFE_TBL_GetAddress(&MyTblPtr, MyTableHandle);
	
	total=MyTblPtr->sample_led1_state+MyTblPtr->sample_led2_state;
